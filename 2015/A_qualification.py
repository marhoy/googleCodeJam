...
def main(fo=sys.stdin):
    ...
    for case in range(1, cases + 1):
        inputs = fo.readline().strip().split()
        string = [int(c) for c in inputs[1]]

        additional_people = 0
        people = 0
        for i, number in enumerate(string, start=1):
            people += number
            # print("At position {}, have {} people".format(i, people))
            missing = max(0, i - people)
            people += missing
            additional_people += missing

        print('Case #{}: {}'.format(case, additional_people))
